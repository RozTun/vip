...
LOGO() {
    ...
    echo -e " ───│    $Green├─┤│ │ │ │ │└─┐│  ├┬┘│├─┘ │   │  │ │ ├┤ $NC   │───"
    echo -e " ───│    $Green┴ ┴└─┘ ┴ └─┘└─┘└─┘┴└─┴┴   ┴   ┴─┘┴ ┴ └─┘$NC   │───"
    echo -e "    │    ${YELLOW}Copyright${FONT} (C)${GRAY}https://t.me/RozTun$NC   │"
    echo -e "    └───────────────────────────────────────────────┘"
...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To RozTun Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}TekiroVpn ${NC}${YELLOW}(${NC} ${green}Jabar Putra ${NC}${YELLOW})${NC}"
echo -e "       © SMVPN & RozTun ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
data_ip="https://raw.githubusercontent.com/RozTun/vip/main/ip"
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/RozTun/vip/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/RozTun/vip/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/RozTun/vip/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/RozTun/vip/main/"
...
#install cf
wget ${REPO}レスキセティワン/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
<i>TekiroVpn</i>
"'&reply_markup={"inline_keyboard":[[{"text":"ORDER SC","url":"https://t.me/RozTun"},{"text":"CONTACK","url":"https://wa.me/6285759375937"}]]}'
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/lynz/limit/shadowsocks/ip
    mkdir -p /etc/limit/ssh
    mkdir -p /etc/limit/vmess
    mkdir -p /etc/limit/vless
    mkdir -p /etc/limit/trojan
    mkdir -p /etc/limit/shadowsocks
    mkdir -p /etc/lynz/log
    mkdir -p /etc/lynz/log/trojan
    mkdir -p /etc/lynz/log/vless
    mkdir -p /etc/lynz/log/vmess
    mkdir -p /etc/lynz/log/ssh
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    # // Ambil Config Server
    wget -O /etc/xray/config.json "${REPO}レスキセティワン/config.json" >/dev/null 2>&1
    #wget -O /usr/local/bin/xray "${REPO}xray/xray.linux.64bit" >/dev/null 2>&1
    wget -O /etc/systemd/system/runn.service "${REPO}レスキセティワン/runn.service" >/dev/null 2>&1
    ...
    wget -O /etc/haproxy/haproxy.cfg "${REPO}レスキセティワン/haproxy.cfg" >/dev/null 2>&1
    wget -O /etc/nginx/conf.d/xray.conf "${REPO}レスキセティワン/xray.conf" >/dev/null 2>&1
    sed -i "s/xxx/${domain}/g" /etc/haproxy/haproxy.cfg
    sed -i "s/xxx/${domain}/g" /etc/nginx/conf.d/xray.conf
    curl ${REPO}レスキセティワン/nginx.conf > /etc/nginx/nginx.conf
...
print_install "Memasang Password SSH"
    wget -O /etc/pam.d/common-password "${REPO}レスキセティワン/password"
...
wget https://aio.tekirovpn.my.id/レスキセティワン/limit.sh && chmod +x limit.sh && ./limit.sh
...
wget -q -O /etc/ssh/sshd_config "${REPO}レスキセティワン/sshd" >/dev/null 2>&1
...
gotop_latest="$(curl -s https://api.github.com/repos/xxxserxxx/gotop/releases | grep tag_name | sed -E 's/.*"v(.*)".*/\1/' | head -n 1)"
    ...
    wget ${REPO}レスキセティワン/bbr.sh &&  chmod +x bbr.sh && ./bbr.sh
